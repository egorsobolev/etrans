  ...
  */
  ...
  for (q = r->nsamp; q < qn; q++) {
    ...
    if (d->x0rnd) {
      ...
      d->chain->equilibrate(d->chain, osc_h, d->x, 0, ps);
    }
	...
	*/
	...
	charge_step_adj(d->ch, h, u, v, sr, si, &nskip, &h0, &sn, &cs, d->ch->s0);
	if (h0 > mx) mx = h0;
	if (h0 < mn) mn = h0;
	...
	*/
	...
	b2r[k] = sr[k] * sr[k] + si[k] * si[k];
      ...
      d->chain->step_coupled(d->chain, h, x, b2l, b2r);
      b2t = b2l;
      b2l = b2r;
      b2r = b2t;
    ...
    if (!rank) {
      ...
      */
      printf("%6d %8.2f %9.2g %9.6g %9.6g\n", q + 1, wtm1 - wtm0, (double) nr, (double) mn, (double) mx);
      fflush(stdout);
    }
    elapse += (wtm1 - wtm0);
    wtm0 = wtm1;
