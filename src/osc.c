...
void runge(oscilator_t *o, float h, float *x)
{
	...
	vsRngGaussian(VSL_METHOD_SGAUSSIAN_BOXMULLER2, o->rstr, o->n, x + o->n, 0.0f, 1.0f);
	/*memset(x + o->n, 0, o->n * sizeof(float));*/

	for (i = o->n; i < m; ++i)
		x[i] = sqrth * o->sig[i - o->n] * x[i] + y[i];
	f(o, x, k1);
	cblas_saxpy(m, h, k1, 1, y, 1);
	f(o, y, k2);
	cblas_saxpy(m, h_2, k1, 1, x, 1);
	cblas_saxpy(m, h_2, k2, 1, x, 1);
}
...
void osc_integrate(oscilator_t *osc, float h, float *x, int nskip, int nout, float *u, float *v)
{
	...
	for (j = 0; j < nout; ++j) {
		for (i = 0; i < nskip; ++i)
			runge(osc, h, x);
		cblas_scopy(osc->n, x, 1, u + k, 1);
		cblas_scopy(osc->n, x + osc->n, 1, v + k, 1);
		k += osc->n;
	}
}
...
void osc_x0_rand(oscilator_t *o, float *x)
{
    int i;
    if (o->kt == 0.0f) {
        memset(x, 0, 2 * o->n * sizeof(float));
        return;
    }
	
    vsRngGaussian(VSL_METHOD_SGAUSSIAN_BOXMULLER2, o->rstr, 2 * o->n, x, 0.0f, sqrtf(o->kt/2.0));
    for (i = 0; i < o->n; ++i) {
       if (o->upr[i] == 0.0f)
	   x[i] = x[i + o->n] = 0.0f;
       else
...
{
	static float e0 = 0.261838952f;
	static float t0 = 100.0f;
	int i, a;
	int seed;
	struct timeval tm;
	pid_t pid;
	...
	o->n = n;
	o->n1 = n - 1;
	o->xi = malloc(11 * n * sizeof(float));
	if (!o->xi)
		return 1;
   
        o->period = 2.0f * M_PI / sqrtf(upr);
	...
	for (i = 0; i < n; ++i) {
		a = o->upr[i] != 0.0;
		o->sig[i] = (float) a * sqrtf(o->kt * tren);
		o->nsite += a;
	}
	return 0;
}
